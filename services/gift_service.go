...
func (s *giftService) GetAllUse(useCache bool) []models.ObjGiftPrize {
	...
	if list != nil {
		...
		for _, gift := range list {
			//PrizeCode 存储形式是 xxx-yyy
			codes := strings.Split(gift.PrizeCode, "-")
			if len(codes) == 2 {
				// 设置了获奖编码范围 a-b 才可以进行抽奖
				codeA := codes[0]
				codeB := codes[1]
				a, e1 := strconv.Atoi(codeA) //string --> int
				b, e2 := strconv.Atoi(codeB) //string --> int
				if e1 == nil && e2 == nil && b >= a && a >= 0 && b < 10000 {
					data := models.ObjGiftPrize{
						...
						Gtype:        gift.Gtype, //奖品类型
						Gdata:        gift.Gdata, //奖品扩展数据，入虚拟币的数量
					}
...
func (s *giftService) getAllByCache() []models.LtGift {
	...
	//rs是interface{}类型
	str := comm.GetString(rs, "") //将interface{}类型转换成string类型
	if str == "" {
		return nil
	}
	// 将json数据反序列化
	datalist := []map[string]interface{}{}
	//str本身就是[{},{},{}] 一个slice里面存多个map结构（js对象）
	...
	for i := 0; i < len(datalist); i++ {
		//取出里面每个json , data是一个map
		data := datalist[i]
		//从map结构里面获取指定KEY，默认值是0
