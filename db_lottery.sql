...
CREATE TABLE `lt_gift` (
  ...
  `prize_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '发奖周期，D天', /*表示中奖之后多久发放*/
  ...
  `prize_data` mediumtext COMMENT '发奖计划，[[时间1,数量1],[时间2,数量2]]',  /*mediumtext表示超长文本*/
...
CREATE TABLE `lt_userday` (
  ...
  `sys_updated` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '修改时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uid_day` (`uid`,`day`) /*组合唯一索引*/
