...
func (c *lotteryController) GetLucky() map[string]interface{} {
	...
	result["success"] = ok
	//这里的抽奖过程并不是随机，而是先把一个池的奖品抽完才会去抽下一个奖品池
	for _, data := range giftlist {
		...
		if data.rateMin <= int(code) && data.rateMax > int(code) {
			...
			sendData := ""
			switch data.gtype {
			case giftTypeCoin: //虚拟币
				ok, sendData = sendCoin(data)
			case giftTypeCoupon:  //优惠券，不相同的编码
				ok, sendData = sendCoupon(data)
			case giftTypeCouponFix:  //优惠券，相同的编码
				ok, sendData = sendCouponFix(data)
			case giftTypeRealSmall:  //实物小奖
				ok, sendData = sendRealSmall(data)
			case giftTypeRealLarge:  //实物大奖
