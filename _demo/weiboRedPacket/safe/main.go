...
//var mu sync.Mutex
//使用缓存通道只是保证发送给处理池不会阻塞，但是处理池每次都只有一个处理程序在运行，所以不算是并发
var chTasks  = make(chan task, 10)
		...
		if ok && l != nil {
			...
			if len(list) > 1 {
				if i == len(list) - 1 {
					...
					//append(list[:i], list[i+1:]...)不是并发安全的，虽然每次i都是不同的，但是多个go同时拼接slice不是并发安全的
					//mu.Lock()
					valueList := append(list[:i], list[i+1:]...)
					//mu.Unlock()
					packageList.Store(uint32(id), valueList)
				}
			} else {
...
func (c *lotteryController) GetGet() string {
	...
	//构造一个任务，任务里面有id 和回调channel，取出这个ID的金额
